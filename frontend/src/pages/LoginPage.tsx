...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      // Choose endpoint based on activeTab
      const endpoint = activeTab === "citizen" ? "/api/users/login" : "/api/staff/login";
        ...
        <div className="max-w-md mx-auto">
          ...
          <div className="bg-white shadow-xl rounded-lg p-6">
            <div className="text-center pb-4">
              ...
              <p className="text-slate-600">Sign in to access municipal services and complaint management</p>
            </div>
            ...
            <div className="text-center mb-4">
              <span className={`inline-flex items-center rounded px-2 py-1 text-sm ${
                ...
                {activeTab === "citizen" ? <User className="h-3 w-3 mr-1" /> : <Shield className="h-3 w-3 mr-1" />}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className={`w-full h-12 ${activeTab === "citizen" ? "bg-blue-600 hover:bg-blue-700" : "bg-slate-700 hover:bg-slate-800"} text-white font-medium rounded flex items-center justify-center`}
                disabled={loading}
              >
                {loading ? "Signing In..." : `Sign In as ${activeTab === "citizen" ? "Citizen" : "Staff"}`}
              </button>
            </form>
